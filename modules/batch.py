...
# 예: "(지분:1/2)", "(지분 1/2)", "(지분1/2)", "지분;1/2", "(지분:50%)"
# 주의: 키워드 없는 괄호 분수 (1/2) 등은 본번/부번과 혼동되므로 제외
# [\s:;·.,\-]* → 지분과 값 사이의 각종 구분 문자 허용
_SHARE_SEP = r'[\s:;·.,\-]*'
_ADDR_SHARE_PATTERNS = [
    # 괄호 안: (지분:1/2), (지분;1/2), (지분 50%)
    re.compile(r'\(\s*지분' + _SHARE_SEP + r'(\d+\s*/\s*\d+)\s*\)'),
    re.compile(r'\(\s*지분' + _SHARE_SEP + r'([\d.]+\s*%)\s*\)'),
    # 괄호 없이: 지분:1/2, 지분;1/2, 지분 50%
    re.compile(r'지분' + _SHARE_SEP + r'(\d+\s*/\s*\d+)'),
    re.compile(r'지분' + _SHARE_SEP + r'([\d.]+\s*%)'),
]
