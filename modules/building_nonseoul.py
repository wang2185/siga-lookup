...
# 분할 시군구 매핑: 읍면/동 → 하위 구/출장소 (WeTax 드롭다운용)
# _eup: eup_myeon 필드로 조회 (읍/면 지역)
# _dong: dong 필드로 조회 (동 지역, parse_address가 동 접미사 제거한 형태)
_SIGUNGU_SPLIT = {
    "화성": {
        "_eup": {
            "남양": "만세구", "마도": "만세구", "서신": "만세구",
            "송산": "만세구", "양감": "만세구", "우정": "만세구",
            "장안": "만세구", "팔탄": "만세구", "향남": "만세구",
            "매송": "효행구", "봉담": "효행구", "비봉": "효행구", "정남": "효행구",
        },
        "_dong": {
            "새솔": "만세구",
            "기안": "효행구", "배양": "효행구",
            "기산": "병점구", "능": "병점구", "반월": "병점구", "반정": "병점구",
            "병점": "병점구", "송산": "병점구", "안녕": "병점구", "진안": "병점구",
            "황계": "병점구",
            "금곡": "동탄구", "목": "동탄구", "반송": "동탄구", "방교": "동탄구",
            "산척": "동탄구", "석우": "동탄구", "송": "동탄구", "신": "동탄구",
            "여울": "동탄구", "영천": "동탄구", "장지": "동탄구", "중": "동탄구",
            "청계": "동탄구",
        },
    },
    "부천": {
        "_eup": {},
        "_dong": {
            "도당": "원미구", "상": "원미구", "소사": "원미구", "심곡": "원미구",
            "약대": "원미구", "역곡": "원미구", "원미": "원미구", "중": "원미구",
            "춘의": "원미구",
            "계수": "소사구", "괴안": "소사구", "범박": "소사구", "소사본": "소사구",
            "송내": "소사구", "심곡본": "소사구", "옥길": "소사구",
            "고강": "오정구", "내": "오정구", "대장": "오정구", "삼정": "오정구",
            "여월": "오정구", "오정": "오정구", "원종": "오정구", "작": "오정구",
        },
    },
    "평택": {
        "_eup": {
        ...
        "_dong": {
            ...
            "용이": "", "월곡": "", "유천": "", "죽백": "", "지제": "",
            "청룡": "", "통복": "", "평택": "", "합정": "",
            "가재": "송탄출장소", "고덕": "송탄출장소", "도일": "송탄출장소",
            "독곡": "송탄출장소", "모곡": "송탄출장소", "서정": "송탄출장소",
            "신장": "송탄출장소", "이충": "송탄출장소", "장당": "송탄출장소",
            "장안": "송탄출장소", "지산": "송탄출장소", "칠괴": "송탄출장소",
        },
    },
}
...
class WeTaxModule(BaseLookupModule):
    ...
    def _run_search(self, addr: dict) -> dict:
        ...
        try:
            ...
            # 분할 시군구 자동 판별: sigungu_sub 없으면 eup_myeon/dong으로 구/출장소 결정
            if not addr.get("sigungu_sub") and addr.get("sigungu") in _SIGUNGU_SPLIT:
                split_map = _SIGUNGU_SPLIT[addr["sigungu"]]
                eup = addr.get("eup_myeon", "")
                dong_name = addr.get("dong", "")
                # 1) 읍/면 이름으로 먼저 조회 (팔탄면 → 팔탄 → 만세구)
                gu = split_map.get("_eup", {}).get(eup) if eup else None
                # 2) 읍/면 없으면 dong으로 조회 (병점동 → 병점 → 병점구)
                if gu is None and dong_name:
                    gu = split_map.get("_dong", {}).get(dong_name)
                if gu:
                    sigungu_kw = gu
                    logs.append(f"분할 시군구 감지: {addr['sigungu']} → {gu}")
